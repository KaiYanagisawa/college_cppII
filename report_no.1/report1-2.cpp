...
public:
    ...
    void result(std::ofstream &fout)
    {
        ...
        {
            if (e.intai() > 0)
                fout << "Intai Player======================= (" << std::setw(22) << e.get_r() << std::setw(20) << e.get_n() << std::setw(5) << e.shiro() << "Shou" << std::setw(5) << e.kuro() << "Hai" << std::setw(5) << e.yasumi() << "Kyu"
                     << ")" << std::endl;
            else if (e.yasumi() > 0)
